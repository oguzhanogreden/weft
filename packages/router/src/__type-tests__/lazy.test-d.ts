...
const _app: Node<RouterParamsError, Theme | Router> = RouterApp(themedRouter);

// @ts-expect-error — AC-T2: the lazy component requires `Theme`, so it must appear in R;
...
// @ts-expect-error — the lazy component reads params, so `RouterParamsError` is in E;
// `never` is too narrow.
...
const _staticLazyApp: Node<never, Router> = RouterApp(
  Router.router(_lazyLayout, { notFound: () => h.div({}, "404") }),
);
...
// @ts-expect-error — `oid` decodes from a number; a string is rejected, same as eager.
href(lazyOrder, { path: { oid: "1" } });
