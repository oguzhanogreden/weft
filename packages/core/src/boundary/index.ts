 ...
 * `./boundary-impl` as a namespace — the module form is required because
 * `catch` is a reserved word (exported as `catch` from the module, mirroring
 * Effect 4's own `Effect.catch`), which a `namespace` block cannot express.
 */
