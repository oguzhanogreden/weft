...
- **Server** (`server.ts` → `src/entry-server.ts`): `renderToStringHydratable(App({ initialValue: 3 }))` produces HTML that includes `<!-- stream-start-N -->` / `<!-- stream-end-N -->` comment markers around the reactive `count.changes` region. The server renders the stream's first emission — `3` — between those markers.
