/**
 * Dev SSR server for the server-boundary example.
 *
 * Runs Vite in middleware mode and, per request, renders `<App/>` to hydratable
 * HTML via `entry-server.ts` (running the `Boundary.server` `load` against the
 * server-only `Database`), splices it into `index.html`'s `<!--ssr-outlet-->`
 * placeholder, and serves it. The browser then hydrates the markup with
 * `entry-client.ts`, replaying the inline product payload without re-running
 * `load`.
 */
...
server.listen(PORT, () => {
  console.log(`server-boundary demo running at http://localhost:${PORT}`);
});
