...
props** (`({ path, query }) => …` — the router passes the decoded match straight in),
...
pages reactively) — while keeping the unchanged `Shell` mounted across navigations,
reading typed path/query params, and loading live data (stock) server-side without
...
- **Hydrate** replays that inline payload to seed the resource — no extra request.
