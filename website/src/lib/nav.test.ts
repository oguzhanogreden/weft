...
describe("buildNav", () => {
  it("AC1: produces one group per section in the fixed order guides → concepts → api", () => {
    const { groups } = buildNav([
      doc("api", "core", "@weftui/core", 1),
      doc("guides", "getting-started", "Getting Started", 1),
      doc("concepts", "combinator-api", "The Combinator API", 1),
    ]);
    assert.deepEqual(
    ...
    assert.deepEqual(
      ...
      ["Guides", "Concepts", "API Reference"],
    );
  });

  it("AC2: links within a group are ordered by order, then title", () => {
    const { groups } = buildNav([
      doc("guides", "routing", "Routing", 5),
      doc("guides", "getting-started", "Getting Started", 1),
      doc("guides", "ssr", "Server-Side Rendering", 3),
    ]);
    assert.deepEqual(
      groups[0]?.links.map((l) => l.title),
      ["Getting Started", "Server-Side Rendering", "Routing"],
    );
  });

  it("AC2 edge: equal order falls back to a deterministic title tie-break", () => {
    const { groups } = buildNav([doc("guides", "b", "Beta", 2), doc("guides", "a", "Alpha", 2)]);
    assert.deepEqual(
      groups[0]?.links.map((l) => l.title),
      ["Alpha", "Beta"],
    );
  ...
  it("AC2 edge: absent order (Infinity) sorts last, tie-broken by title", () => {
    const { groups } = buildNav([
      doc("guides", "z", "Zed", Infinity),
      doc("guides", "a", "Apple", Infinity),
  ...
  it("AC3: firstDocPath is the path of the first link in the first group", () => {
    ...
    assert.equal(firstDocPath, "/docs/guides/getting-started");
  });

  it("api links map to /api/:pkg, not /docs/api/:pkg", () => {
    const { groups } = buildNav([doc("api", "router", "@weftui/router", 1)]);
    assert.equal(groups[0]?.links[0]?.path, "/api/router");
  });

  it("AC4: findNav returns the current link and its prev/next neighbours", () => {
    const { findNav } = buildNav([
      doc("guides", "getting-started", "Getting Started", 1),
      doc("guides", "authoring", "Component Authoring", 2),
      doc("guides", "routing", "Routing", 3),
    ]);
    const mid = findNav("/docs/guides/authoring");
  ...
  it("AC4: an unknown path yields no current/prev/next", () => {
    const { findNav } = buildNav([doc("guides", "getting-started", "Getting Started", 1)]);
    assert.deepEqual(findNav("/docs/guides/nope"), {});
  });
